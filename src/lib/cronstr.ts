...
function buildit(parts: Array<string>): string {
  ...
  if (!anyDate) {
    ...
    for (const range of dayOfWeekParams) {
      if (range.valueIsNumber && range.nth > 0) {
        ranges.push(countable_of(range.nth) + ' ' + weekday_name_of(range.valueNumber));
      } else if (range.valueIsNumber && range.step > 0) {
        if (range.step === 2) {
          ranges.push('other ' + weekday_name_of(range.valueNumber));
        } else {
          ranges.push(countable_of(range.step) + ' ' + weekday_name_of(range.valueNumber));
        }
      } else if (range.valueIsNumber) {
